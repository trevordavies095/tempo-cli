...
import {
  ...
  settingsUnitPreferenceHttpErrorMessage,
  settingsUnitPreferenceHttpErrorMessageForCli,
  settingsUnitPreferenceHumanSuccessLine,
} from "./settings-unit-preference.js";

const originalFetch = globalThis.fetch;
const SECRET_KEY = "tmp_secret_key_for_tests";

describe("probeSettingsUnitPreference", () => {
  afterEach(() => {
    globalThis.fetch = originalFetch;
    vi.restoreAllMocks();
  });

  it("GET /settings/unit-preference with Authorization Bearer, credentials omit", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(new Response('{"unit":"metric"}', { status: 200 }));
    globalThis.fetch = fetchMock as typeof fetch;

    const result = await probeSettingsUnitPreference(
      "http://localhost:5001",
      SECRET_KEY,
    );
    ...
    expect(url).toBe("http://localhost:5001/settings/unit-preference");
  ...
  it("redacts API key in HTTP error body for CLI message", async () => {
    ...
    if (result.kind === "http") {
      const msg = settingsUnitPreferenceHttpErrorMessageForCli(
        result.status,
        result.body,
        SECRET_KEY,
      );
  ...
  it("maps 404 and 5xx for exit codes", async () => {
    ...
    let result = await probeSettingsUnitPreference(
      "http://localhost:5001",
      SECRET_KEY,
    );
    expect(result.kind).toBe("http");
    ...
    result = await probeSettingsUnitPreference(
      "http://localhost:5001",
      SECRET_KEY,
    );
    expect(result.kind).toBe("http");
...
describe("settingsUnitPreferenceHumanSuccessLine", () => {
  it("renders sorted key: value lines for JSON object", () => {
    const body = JSON.stringify({ unit: "metric" });
    expect(settingsUnitPreferenceHumanSuccessLine(200, body)).toBe(
      "OK (HTTP 200)\nunit: metric",
    );
  });

  it("falls back to plain text for non-JSON body", () => {
    expect(settingsUnitPreferenceHumanSuccessLine(200, "imperial")).toBe(
      "OK (HTTP 200)\nimperial",
    );
  });

  it("returns just the OK header when body is empty", () => {
    expect(settingsUnitPreferenceHumanSuccessLine(200, "")).toBe(
      "OK (HTTP 200)",
    );
  });
});

describe("settingsUnitPreferenceHttpErrorMessage", () => {
  it("includes path and status", () => {
    expect(settingsUnitPreferenceHttpErrorMessage(401, "")).toBe(
      "GET /settings/unit-preference returned 401",
    );
  });

  it("appends a snippet of the body when present", () => {
    expect(settingsUnitPreferenceHttpErrorMessage(500, "boom")).toBe(
      "GET /settings/unit-preference returned 500: boom",
    );
  });
});
