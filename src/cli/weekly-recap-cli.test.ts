...
describe("tempo weekly-recap (subprocess)", () => {
  it("exits 1 with MISSING_API_KEY JSON on stderr when no key is configured", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-weekly-recap-"));
    ...
    const r = runTempo(["--output", "json", "weekly-recap"], env);
    ...
    expect(err.error.message).toContain("weekly-recap");
  });
});
