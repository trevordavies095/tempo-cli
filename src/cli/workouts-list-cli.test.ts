...
describe("tempo workouts list (subprocess)", () => {
  it("exits 1 with MISSING_API_KEY JSON on stderr when no key is configured", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-workouts-list-"));
    ...
    const r = runTempo(["--output", "json", "workouts", "list"], env);
    ...
    expect(err.error.message).toContain("workouts list");
  });
});
