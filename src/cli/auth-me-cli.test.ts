...
describe("tempo auth me (subprocess)", () => {
  it("exits 1 with MISSING_API_KEY JSON on stderr when no key is configured", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-auth-me-"));
    ...
    const r = runTempo(["--output", "json", "auth", "me"], env);
    ...
    expect(err.error.message).toContain("auth me");
  });
});
