...
describe("tempo workout media list (subprocess)", () => {
  it("exits 1 with MISSING_API_KEY when no key is configured", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-media-list-"));
    ...
    const r = runTempo(
      ["--output", "json", "workout", "media", "list", SAMPLE_UUID],
      env,
    );
    ...
    expect(err.error.message).toContain("media list");
  });

  it("exits 1 with INVALID_ARGUMENTS when id is not a UUID", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-media-list-badid-"));
    const tempoDir = join(base, "tempo");
    mkdirSync(tempoDir, { recursive: true });
    writeFileSync(
      join(tempoDir, "config.toml"),
    ...
    const r = runTempo(
      ["--output", "json", "workout", "media", "list", "bad-id"],
      env,
    );
    expect(r.status).toBe(1);
    const err = JSON.parse(r.stderr.trim()) as {
