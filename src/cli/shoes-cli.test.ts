...
describe("tempo shoes (subprocess)", () => {
  it("shoes list exits 1 with MISSING_API_KEY JSON on stderr when no key is configured", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-shoes-list-"));
    ...
    const r = runTempo(["--output", "json", "shoes", "list"], env);
    ...
    expect(err.error.message).toContain("shoes list");
  });

  it("shoes mileage exits 1 with INVALID_ARGUMENTS when id is not a UUID", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-shoes-mileage-"));
    const tempoDir = join(base, "tempo");
    mkdirSync(tempoDir, { recursive: true });
    writeFileSync(
      join(tempoDir, "config.toml"),
    ...
    const r = runTempo(
      ["--output", "json", "shoes", "mileage", "not-a-uuid"],
      env,
    );
    expect(r.status).toBe(1);
    expect(r.stdout).toBe("");
    ...
    expect(err.error.message).toContain("not-a-uuid");
