...
describe("tempo workout get (subprocess)", () => {
  it("exits 1 with MISSING_API_KEY JSON on stderr when no key is configured", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-workout-get-"));
    ...
    expect(err.error.message).toContain("workout get");
  });

  it("exits 1 with INVALID_ARGUMENTS when id is not a UUID", () => {
    const base = mkdtempSync(join(tmpdir(), "tempo-cli-workout-get-badid-"));
    const tempoDir = join(base, "tempo");
    mkdirSync(tempoDir, { recursive: true });
    writeFileSync(
      join(tempoDir, "config.toml"),
    ...
    const r = runTempo(
      ["--output", "json", "workout", "get", "not-a-uuid"],
      env,
    );
    expect(r.status).toBe(1);
    expect(r.stdout).toBe("");
